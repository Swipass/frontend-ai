...
export default defineConfig(({ mode }) => {
  // Search-engine verification tokens, read at build time (Vercel env or .env).
  const env = loadEnv(mode, process.cwd(), 'SEO_')
  ...
  return {
    plugins: [
      ...
      seoPlugin({
        googleVerification: env.SEO_GOOGLE_SITE_VERIFICATION?.trim(),
        bingVerification: env.SEO_BING_SITE_VERIFICATION?.trim(),
      }),
    ],
    build: {
